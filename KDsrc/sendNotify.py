...
if "BARK_ICON" in os.environ and os.environ["BARK_ICON"]:
    BARK_ICON = os.environ["BARK_ICON"]
...
if "BARK_GROUP" in os.environ and os.environ["BARK_GROUP"]:
    BARK_GROUP = os.environ["BARK_GROUP"]
...
def bark(title, content):
    ...
    if BARK:
        try:
            ...
            f"""https://api.day.app/{BARK}/{title}/{urllib.parse.quote_plus(content)}?sound={BARK_SOUND}&icon={BARK_ICON}&group={BARK_GROUP}""").json()
    ...
    if BARK_PUSH:
        try:
            response = requests.get(
            f"""{BARK_PUSH}/{title}/{urllib.parse.quote_plus(content)}?sound={BARK_SOUND}&icon={BARK_ICON}&group={BARK_GROUP}""").json()
            if response['code'] == 200:
                print('推送成功！')
            else:
                print('推送失败！')
